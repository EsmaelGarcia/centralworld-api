...
router.post('/collectiondetails', auth, async (req, res) => {
  console.log('request parameters are ', req.body)
  ...
  console.log('klklklklklklkllllkllklklklklklklklklklklklklklklk')
  try {
  ...
  console.log('mnmnmnmnmnmnmnmnmnnmnmnmnmnmnmnmnmnmnmnmnmnmnmnm')
  ...
  let is1155 = await isValidERC1155(erc721Address);
  console.log('am i 1155 ? ', is1155);
  let isInternal = await FactoryUtils.isInternalCollection(
    erc721Address,
    !is1155
  );
  console.log('am i Internal? ', isInternal);
...
router.post('/isValidated', auth, async (req, res) => {
  try {
    ...
    // let request = `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${erc721Address}&apikey=${etherScanApiKey}`;
    let request = `https://api-ropsten.etherscan.io/api?module=contract&action=getsourcecode&address=${erc721Address}&apikey=${etherScanApiKey}`;
